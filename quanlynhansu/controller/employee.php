<?php
	...
	if(isset($_POST['submit']))
	{
		...
		{
			header("location:../employeeadd.php?msg=Email đã tồn tại!");exit;
		}
		else
		{
			if(!empty($_FILES['pfimg']['name']))
			{
				...
				{
					header("location:../employeeadd.php?msg=Ảnh không tồn tại !");exit;
				}
				else
				{
					if($size > 1000000)
					{
						header("location:../employeeadd.php?msg=Yêu cầu kích thước tệp tối đa 1MB! ");exit;
					}
					else
					{	
						$ImageComplete=true;
			...
			else
			{
				...
				{
					header("location:../employeeadd.php?msg=Hãy chọn ảnh đại diện! ");exit;	
				}
			}	
		}
